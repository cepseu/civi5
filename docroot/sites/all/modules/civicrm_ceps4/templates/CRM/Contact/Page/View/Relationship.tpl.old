    ...
    {* show browse table for any action *}
      <div id="current-relationships">
        ...
        {foreach from=$currentRelationships item=rel}
            {crmAPI var="contact" entity="contact" action="get" version="3" contact_id=$rel.cid return="custom_3,custom_30"}
       ...
       <a href="{crmURL p='civicrm/contact/view' q="action=view&reset=1&cid=`$rel.cid`"}">{$rel.name}</a>
          ...
          ($contactId eq $rel.contact_id_b and $rel.is_permission_b_a eq 1) }
            ...
            {/if}
                <td>{$rel.job_title}</td>
                <td>{if ($contact.values[0].custom_3 eq 1)}Yes{/if}</td>
                <td>{if ($contact.values[0].custom_30 eq 1)}Yes{/if}</td>
